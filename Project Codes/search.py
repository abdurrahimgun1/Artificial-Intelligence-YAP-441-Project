...
def breadthFirst(initial_node, window,
                 deadlock_detection=False):  # initial_node is a Node object representing the initial state of the puzzle

    # Check if the start element is the goal
    if initial_node.state.isGoal(
            Node.tab_stat):  # Node.tab_stat is a 2D array of strings representing the state of the puzzle
    ...
    while True:  # Loop until the goal is found or the OPEN queue is empty
        ...
        label12 = Label(window, text=f'*** Step {step} ***', bg='#c45242',
                        fg='white')  # erreur de pas ecrire au meme temps de recherche
        label12.pack()
        window.update()
...
def Astar(init_node, window, heuristique=1, deadlock_detection=False):
    ...
    while True:  # Loop until the goal is found or the OPEN queue is empty
        ...
        label123 = Label(window, text=f'*** Step {step} ***', bg='#c45242',
                         fg='white')  # erreur de pas ecrire au meme temps de recherche
        label123.pack()
        window.update()
...
def IDAstar(init_node, window, heuristique=1, deadlock_detection=False):
    global step
    global label123
    ...
    if heuristique == 1:
        ida_threshold = init_node.heuristic1()
    elif heuristique == 2:
        ida_threshold = init_node.heuristic2()
    else:
        ida_threshold = init_node.heuristic3()

    while True:
        current, search_result = ida_search_function(init_node, window, 0, ida_threshold, heuristique,
                                                     deadlock_detection)
        if (search_result == -1):
            label123.destroy()
            return current, step
...
def ida_search_function(init_node, window, g, ida_threshold, heuristique, deadlock_detection):
    global step
    global label123
    ...
    if heuristique == 1:
        f_value = g + init_node.heuristic1()
    elif heuristique == 2:
        f_value = g + init_node.heuristic2()
    else:
        f_value = g + init_node.heuristic3()

    if (f_value > ida_threshold):
        return init_node, f_value
    if init_node.state.isGoal(Node.tab_stat):
        label123.destroy()
        return init_node, -1
    elif deadlock_detection:
        if init_node.state.isDeadLock(Node.deadlock_map):
            return init_node, ida_threshold

    min = float("inf")
    succ = init_node.succ()
    while len(succ) != 0:
        child = succ.popleft()
        init_node, tempValue = ida_search_function(child, window, g + 1, ida_threshold, heuristique, deadlock_detection)
        if (tempValue == -1):
            return init_node, -1
        if (tempValue < min):
            min = tempValue
    return init_node, min
